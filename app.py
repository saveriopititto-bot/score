...
from ui.feedback import render_feedback_form  # <--- NUOVO IMPORT FEEDBACK
...
# --- CASO B: DASHBOARD (Loggato) ---
    ...
    # --- DEBUG TOOL (Opzionale: puoi rimuoverlo in produzione) ---
    with st.expander("🕵️‍♂️ Debug Dati Strava (Solo Sviluppo)", expanded=False):
        st.write("Dati Profilo:", ath)
        if "strava_zones" in st.session_state:
            st.write("Dati Zone:", st.session_state.strava_zones)
            ...
            if zones_data:
                ...
                if hr_zones:
                    ...
                    if extracted_max and extracted_max > 0: 
                        ...
                        if age > 0: hr_max = int(208 - (0.7 * age)) # Formula Tanaka

                # Fix FTP (Reverse Engineering)
    ...
    if st.session_state.data:
        ...
        if df.empty:
            ...
            # --- FOOTER LEGALE ---
            st.markdown("<br>", unsafe_allow_html=True)
            render_legal_section()import streamlit as st
import pandas as pd
import time
